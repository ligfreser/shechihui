 package com.luxplat.core.tools.database;
...
import org.springframework.stereotype.Repository;
   ...
   implements IBackup
 {
     ...
     throws Exception
   {
     ...
     List backup_list = new ArrayList();
     ...
     try
     {
       ...
       for (localIterator1 = backup_list.iterator(); localIterator1.hasNext(); 
         localIterator2.hasNext())
       {
         String table = (String)localIterator1.next();
         StringBuilder strBuilder = new StringBuilder();
         strBuilder.append("show create table ").append(table);
         List list = this.publicMethod.getAllColumns(strBuilder
           .toString());
     ...
     try {
       ...
       for (String table : (String[])backup_list.toArray()) {
     ...
     throws Exception
   {
     ...
     try {
       ...
       for (String sqlStr : (String[])sqlStrList.toArray()) {
     ...
     throws Exception
   {
     ...
     try
     {
       ...
       StringBuilder columnsStr = new StringBuilder().append("describe ").append(
         tableName);
       List columnList = this.publicMethod.getDescribe(columnsStr.toString());
       sqlStr.append("SELECT ");
       for (TableColumn bColumn : (TableColumn[])columnList.toArray())
       {
       ...
       while (rs.next())
       {
         ...
         for (TableColumn dbColumn : (TableColumn[])columnList.toArray()) {
   ...
   public boolean export(String tables, String path)
   {
     ...
     try {
       ...
       pwrite.println(AppendMessage.headerMessage());
       pwrite.println(AppendMessage.insertHeaderMessage());
       List list = Arrays.asList(tables.split(","));
       List insertList = null;
       int i = 0;
       for (Iterator localIterator = list.iterator(); localIterator.hasNext()||i < insertList.size();)
       {
         String table = (String)localIterator.next();
 
         insertList = getAllDatas(table.toString());
         i = 0; 
         pwrite.flush();
         pwrite.println((String)insertList.get(i));
