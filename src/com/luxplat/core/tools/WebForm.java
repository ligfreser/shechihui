 package com.luxplat.core.tools;
 
 import com.easyjf.beans.BeanUtils;
import com.easyjf.beans.BeanWrapper;
import com.luxplat.core.annotation.Lock;
 ...
 public class WebForm
 {
   ...
   public void toQueryPo(HttpServletRequest request, QueryObject qo, Class classType, ModelAndView mv)
   {
     ...
     try {
       ...
       Iterator keyes;
       for ( Iterator value = maps.iterator(); value.hasNext();)
       {
         Map m = (Map)value.next();
         keyes = m.keySet().iterator();
         ...
         if (field.indexOf("q_") == 0) {
           ...
           PropertyDescriptor[] entity_propertys = null;
           if (field.indexOf(".") > 0) {
             Class entity = Class.forName("com.luxplat.domain." + 
           ...
           boolean add = false;
           int ep_len = entity_propertys.length; 
           for (int i = 0; i < ep_len; i++) { 
        	   PropertyDescriptor pd1 = entity_propertys[i];
             if (field.indexOf(".") < 0) {
               if (pd1.getName().equals(field.substring(2))) {
                 add = true;
               }
 
             }
             else if (pd1.getName().equals(
               field.subSequence(
