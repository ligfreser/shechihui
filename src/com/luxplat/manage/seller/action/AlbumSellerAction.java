 ...
 import com.luxplat.view.web.tools.AlbumViewTools;
 ...
 public class AlbumSellerAction
 {
   ...
   public String album_del(HttpServletRequest request, String mulitId) {
     ...
     for (String id : ids) {
       if (!id.equals("")) {
         List accs = this.albumService.getObjById(
           Long.valueOf(Long.parseLong(id))).getPhotos();
         for (int i=0 ; i<accs.size() ; i++){
        	 Accessory acc = (Accessory) accs.get(i) ;
             CommUtil.del_acc(request, acc);
             this.databaseTools
               .execute("update luxplat_album set album_cover_id=null where album_cover_id=" + 
               acc.getId());
         }
