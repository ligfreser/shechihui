 ...
 public class TransAreaManageAction
 {
   ...
   private Set<Long> genericIds(TransArea obj)
   {
     ...
     for (int i=0 ; i<obj.getChilds().size() ; i++) {
    	 TransArea child = (TransArea)obj.getChilds().get(i) ;
         Set cids = genericIds(child);
         for (int j=0 ; j<cids.toArray().length ; j++){
        	 Long cid_tmp = (Long)cids.toArray()[j];
             ids.add(cid_tmp);
         }
         ids.add(child.getId());
     }
     ...
     for (String id : ids) {
       if (!id.equals("")) {
         ...
         for (int i=0 ; i<objs.size() ; i++ ){
        	 TransArea obj = (TransArea)objs.get(i) ;
             obj.setParent(null);
