	...
	if (user != null) {
     ...
     for ( sc = user_cart.iterator(); ((Iterator)sc).hasNext(); ) {  
       ...
       for (int i=0 ; i<cart.size() ; i++) {
             ...
             if (tmp_sc1.getStore().getId().equals(((StoreCart) sc).getStore().getId())) {
                 sc_add = false;
               }
       }
       if (sc_add)
     ...
     boolean sc_add;
     for (sc = cookie_cart.iterator(); ((Iterator)sc).hasNext(); ) { 
    	 sc = (StoreCart)((Iterator)sc).next();
       sc_add = true;
       for (int i=0 ; i<cart.size() ; i++) {
    		 StoreCart tmp_sc1 = (StoreCart) cart.get(i) ;
     ...
     if (cart != null) {
       for (sc = cart.iterator(); ((Iterator)sc).hasNext(); ) { 
    	    sc = (StoreCart)((Iterator)sc).next();
         if (sc != null) {
           list.addAll(((StoreCart) sc).getGcs());
         }
       }
     }
     float total_price = 0.0F;
     int i = 0 ;GoodsCart gc = null ;
     for (;i<list.size();i++) {
       gc = (GoodsCart)list.get(i) ;
