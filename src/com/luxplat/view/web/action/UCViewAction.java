 ...
 import com.luxplat.uc.api.UCClient;
 ...
 public class UCViewAction
 {
   ...
   private String uc_answer(HttpServletRequest request, HttpServletResponse response)
   {
     ...
     if (action.equals("gettag")) {
       ...
       if (!API_SYNLOGIN)
         return API_RETURN_FORBIDDEN;
       luxplat_login(request, response, get);
     } else if (action.equals("synlogout")) {
       if (!API_SYNLOGOUT)
         return API_RETURN_FORBIDDEN;
       luxplat_logout(request, response, get); } else {
       if (action.equals("updateclient")) {
         ...
         if (!API_UPDATEPW)
           return API_RETURN_FORBIDDEN;
         luxplat_update_pws(request, response, get);
         return API_RETURN_SUCCEED;
   ...
   protected void luxplat_login(HttpServletRequest request, HttpServletResponse response, Map<String, String> args)
   {
     ...
     if (!admin_login) {
       ...
       String url = CommUtil.getURL(request) + 
         "/luxplat_login.htm?username=" + 
         CommUtil.encode(userName) + "&password=" + 
         "luxplat_thid_login_" + password + "&encode=true";
       try {
   ...
   protected void luxplat_logout(HttpServletRequest request, HttpServletResponse response, Map<String, String> args)
   {
     String url = CommUtil.getURL(request) + "/luxplat_logout.htm";
     try {
       response.sendRedirect(url);
     }
     catch (IOException e) {
   ...
   protected void luxplat_update_pws(HttpServletRequest request, HttpServletResponse response, Map<String, String> args)
   {
